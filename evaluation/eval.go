...
func Evaluate(pos *Position) int {
	...
	for file := Max(File(whiteKingLocation)-1, FILE_A); file <= Min(File(whiteKingLocation)+1, FILE_H); file++ {
		ours := pos.Pawns & FILES[file] & pos.White
		var ourDist int
		if ours == 0 {
			ourDist = 7
		} else {
		...
		theirs := pos.Pawns & FILES[file] & pos.Black
		var theirDist int
		if theirs == 0 {
			theirDist = 7
		} else {
			theirDist = Abs(Rank(whiteKingLocation) - Rank(MostSignificantBit(theirs)))
		}
	...
	for file := Max(File(blackKingLocation)-1, FILE_A); file <= Min(File(blackKingLocation)+1, FILE_H); file++ {
		ours := pos.Pawns & FILES[file] & pos.Black
		var ourDist int
		if ours == 0 {
			ourDist = 7
		} else {
		...
		theirs := pos.Pawns & FILES[file] & pos.White
		var theirDist int
		if theirs == 0 {
			theirDist = 7
		} else {
			theirDist = Abs(Rank(blackKingLocation) - Rank(BitScan(theirs)))
		}
